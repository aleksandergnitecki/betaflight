...
PG_RESET_TEMPLATE(
    pidConfig_t, pidConfig, .pid_process_denom = DEFAULT_PID_PROCESS_DENOM,
    ...
    .runaway_takeoff_deactivate_delay =
        500, // Accumulated time (in milliseconds) before deactivation in
             // successful takeoff
);
#else
...
#define ACRO_TRAINER_LOOKAHEAD_RATE_LIMIT                                      \
  500.0f // Max gyro rate for lookahead time scaling
#define ACRO_TRAINER_SETPOINT_LIMIT 1000.0f // Limit the correcting setpoint
#endif                                      // USE_ACRO_TRAINER

#define CRASH_RECOVERY_DETECTION_DELAY_US                                      \
  1000000 // 1 second delay before crash recovery detection is active after
          // entering a self-level mode

#define LAUNCH_CONTROL_YAW_ITERM_LIMIT                                         \
  50 // yaw iterm windup limit when launch mode is "FULL" (all axes)
...
PG_REGISTER_ARRAY_WITH_RESET_FN(pidProfile_t, PID_PROFILE_COUNT, pidProfiles,
                                PG_PID_PROFILE, 11);
...
void resetPidProfile(pidProfile_t *pidProfile) {
  RESET_CONFIG(
      ...
      .pidSumLimit = PIDSUM_LIMIT, .pidSumLimitYaw = PIDSUM_LIMIT_YAW,
      .yaw_lowpass_hz = 100, .dterm_notch_hz = 0, .dterm_notch_cutoff = 0,
      .itermWindup =
          80, // sets iTerm limit to this percentage below pidSumLimit
      .pidAtMinThrottle = PID_STABILISATION_ON, .angle_limit = 60,
      .feedforward_transition = 0, .yawRateAccelLimit = 0, .rateAccelLimit = 0,
      ...
      .horizon_limit_degrees = 135, .horizon_ignore_sticks = false,
      .crash_limit_yaw = 200, .itermLimit = 400, .throttle_boost = 5,
      .throttle_boost_cutoff = 15, .iterm_rotation = false,
      .iterm_relax = ITERM_RELAX_RP,
      .iterm_relax_cutoff = ITERM_RELAX_CUTOFF_DEFAULT,
      .iterm_relax_type = ITERM_RELAX_SETPOINT, .acro_trainer_angle_limit = 20,
      .acro_trainer_lookahead_ms = 50, .acro_trainer_debug_axis = FD_ROLL,
      .acro_trainer_gain = 75, .abs_control_gain = 0, .abs_control_limit = 90,
      .abs_control_error_limit = 20, .abs_control_cutoff = 11,
      .dterm_lpf1_static_hz = DTERM_LPF1_DYN_MIN_HZ_DEFAULT,
      // NOTE: dynamic lpf is enabled by default so this setting is actually
      // overridden and the static lowpass 1 is disabled. We can't set this
      // value to 0 otherwise Configurator versions 10.4 and earlier will also
      // reset the lowpass filter type to PT1 overriding the desired BIQUAD
      // setting.
      .dterm_lpf2_static_hz =
          DTERM_LPF2_HZ_DEFAULT, // second Dterm LPF ON by default
      .dterm_lpf1_type = FILTER_PT1, .dterm_lpf2_type = FILTER_PT1,
      ...
      .launchControlThrottlePercent = 20, .launchControlAngleLimit = 0,
      .launchControlGain = 40, .launchControlAllowTriggerReset = true,
      .use_integrated_yaw = false, .integrated_yaw_relax = 200,
      .thrustLinearization = 0, .d_max = D_MAX_DEFAULT, .d_max_gain = 37,
      .d_max_advance = 20, .motor_output_limit = 100,
      .auto_profile_cell_count = AUTO_PROFILE_CELL_COUNT_STAY,
      .transient_throttle_limit = 0, .profileName = {0}, .dyn_idle_min_rpm = 0,
      .dyn_idle_p_gain = 50, .dyn_idle_i_gain = 50, .dyn_idle_d_gain = 50,
      .dyn_idle_max_increase = 150,
      .feedforward_averaging = FEEDFORWARD_AVERAGING_2_POINT,
      .feedforward_max_rate_limit = 90, .feedforward_smooth_factor = 65,
      .feedforward_jitter_factor = 7, .feedforward_boost = 15,
      .dterm_lpf1_dyn_expo = 5, .level_race_mode = false,
      ...
      .anti_gravity_cutoff_hz = 5, .anti_gravity_p_gain = 100,
      .tpa_mode = TPA_MODE_D, .tpa_rate = 65, .tpa_breakpoint = 1350,
      .angle_feedforward_smoothing_ms = 80, .angle_earth_ref = 100,
      .horizon_delay_ms =
          500, // 500ms time constant on any increase in horizon strength
      .tpa_low_rate = 20, .tpa_low_breakpoint = 1050, .tpa_low_always = 0,
      .ez_landing_threshold = 25, .ez_landing_limit = 15,
      .ez_landing_speed = 50, .spa_center = {0, 0, 0}, .spa_width = {0, 0, 0},
      .spa_mode = {0, 0, 0},
      ...
      .feedforward_yaw_hold_time =
          100, // a value of 100 is a time constant of about 100ms, and is OK
               // for a 5in; smaller values decay faster, eg for smaller props
      .tpa_curve_type = TPA_CURVE_CLASSIC, .tpa_curve_stall_throttle = 30,
      .tpa_curve_pid_thr0 = 200, .tpa_curve_pid_thr100 = 70,
      .tpa_curve_expo = 20, .tpa_speed_type = TPA_SPEED_BASIC,
      .tpa_speed_basic_delay = 1000, .tpa_speed_basic_gravity = 50,
      .tpa_speed_adv_prop_pitch = 370, .tpa_speed_adv_mass = 1000,
      .tpa_speed_adv_drag_k = 1000, .tpa_speed_adv_thrust = 2000,
      .tpa_speed_max_voltage = 2520, .tpa_speed_pitch_offset = 0,
      .yaw_type = YAW_TYPE_RUDDER, .angle_pitch_offset = 0,
      .chirp_lag_freq_hz = 3, .chirp_lead_freq_hz = 30,
      .chirp_amplitude_roll = 230, .chirp_amplitude_pitch = 230,
      .chirp_amplitude_yaw = 180, .chirp_frequency_start_deci_hz = 2,
      .chirp_frequency_end_deci_hz = 6000, .chirp_time_seconds = 20, );
}
...
static float calcWingThrottle(void) {
  ...
  if (pidRuntime.tpaSpeed.maxVoltage > 0.0f) {
    batteryThrottleFactor =
        getBatteryVoltageLatest() / 100.0f / pidRuntime.tpaSpeed.maxVoltage;
...
static float calcWingAcceleration(float throttle, float pitchAngleRadians) {
  ...
  const float thrust =
      (throttle * throttle - throttle * tpa->speed * tpa->inversePropMaxSpeed) *
      tpa->twr * G_ACCELERATION;
...
static float calcWingTpaArgument(void) {
  ...
  DEBUG_SET(
      DEBUG_TPA, 3,
      lrintf(t * 1000.0f)); // calculated throttle in the range of 0 - 1000

  // pitchRadians is always -90 to 90 degrees. The bigger the ABS(pitch) the
  // less portion of pitchOffset is needed. If ABS(roll) > 90 degrees - flying
  // inverted, then negative portion of pitchOffset is needed. If ABS(roll) ~ 90
  // degrees - flying sideways, no pitchOffset is applied.
  const float correctedPitchAnge =
      pitchRadians + cos_approx(pitchRadians) * cos_approx(rollRadians) *
                         pidRuntime.tpaSpeed.pitchOffset;
  ...
  const float tpaArgument = constrainf(
      pidRuntime.tpaSpeed.speed / pidRuntime.tpaSpeed.maxSpeed, 0.0f, 1.0f);
...
static void updateStermTpaFactor(int axis, float tpaFactor) {
  ...
  if (pidRuntime.tpaCurveType == TPA_CURVE_HYPERBOLIC) {
    const float maxSterm =
        tpaFactorSterm * (float)currentPidProfile->pid[axis].S * S_TERM_SCALE;
    if (maxSterm > 1.0f) {
...
static float wingAdjustSetpoint(float currentPidSetpoint, int axis) {
  ...
  if (!IS_AXIS_IN_ANGLE_MODE(axis)) {
    const bool skipYaw =
        axis == FD_YAW && currentPidProfile->yaw_type == YAW_TYPE_DIFF_THRUST;
    if (pidRuntime.tpaFactorSterm[axis] > 0.0f && pidRuntime.tpaFactor > 0.0f &&
        !skipYaw) {
      adjustedSetpoint = currentPidSetpoint * pidRuntime.tpaFactorSterm[axis] /
                         pidRuntime.tpaFactor;
    }
  }
...
static float getTpaFactorClassic(float tpaArgument) {
  ...
  bool isThrottlePastTpaLowBreakpoint =
      (tpaArgument >= pidRuntime.tpaLowBreakpoint ||
       pidRuntime.tpaLowBreakpoint <= 0.01f);
  float tpaRate = 0.0f;
  if (isThrottlePastTpaLowBreakpoint || isTpaLowFaded) {
    tpaRate = pidRuntime.tpaMultiplier *
              fmaxf(tpaArgument - pidRuntime.tpaBreakpoint, 0.0f);
    if (!pidRuntime.tpaLowAlways && !isTpaLowFaded) {
      isTpaLowFaded = true;
    }
  } else {
    tpaRate = pidRuntime.tpaLowMultiplier *
              (pidRuntime.tpaLowBreakpoint - tpaArgument);
  }
...
void pidUpdateAntiGravityThrottleFilter(float throttle) {
  ...
  float throttleDerivative =
      fabsf(throttle - previousThrottle) * pidRuntime.pidFrequency;
  ...
  throttleDerivative =
      pt2FilterApply(&pidRuntime.antiGravityLpf, throttleDerivative);
...
float pidCompensateThrustLinearization(float throttle) {
  if (pidRuntime.thrustLinearization != 0.0f) {
    ...
    throttle /=
        1.0f + pidRuntime.throttleCompensateAmount * sq(throttleReversed);
  }
  return throttle;
}
...
STATIC_UNIT_TESTED FAST_CODE_NOINLINE float calcHorizonLevelStrength(void) {
  const float currentInclination =
      MAX(abs(attitude.values.roll), abs(attitude.values.pitch)) * 0.1f;
  // 0 when level, 90 when vertical, 180 when inverted (degrees):
  float absMaxStickDeflection =
      MAX(fabsf(getRcDeflection(FD_ROLL)), fabsf(getRcDeflection(FD_PITCH)));
  // 0-1, smoothed if RC smoothing is enabled

  float horizonLevelStrength =
      MAX((pidRuntime.horizonLimitDegrees - currentInclination) *
              pidRuntime.horizonLimitDegreesInv,
          0.0f);
  // 1.0 when attitude is 'flat', 0 when angle is equal to, or greater than,
  // horizonLimitDegrees
  horizonLevelStrength *=
      MAX((pidRuntime.horizonLimitSticks - absMaxStickDeflection) *
              pidRuntime.horizonLimitSticksInv,
          pidRuntime.horizonIgnoreSticks);
  ...
  if (pidRuntime.horizonDelayMs) {
    const float horizonLevelStrengthSmoothed =
        pt1FilterApply(&pidRuntime.horizonSmoothingPt1, horizonLevelStrength);
    horizonLevelStrength =
        MIN(horizonLevelStrength, horizonLevelStrengthSmoothed);
  }
  return horizonLevelStrength;
  // 1 means full levelling, 0 means none
}

// Use the FAST_CODE_NOINLINE directive to avoid this code from being inlined
// into ITCM RAM to avoid overflow. The impact is possibly slightly slower
// performance on F7/H7 but they have more than enough processing power that it
// should be a non-issue.
STATIC_UNIT_TESTED FAST_CODE_NOINLINE float
pidLevel(int axis, const pidProfile_t *pidProfile,
         const rollAndPitchTrims_t *angleTrim, float currentPidSetpoint,
         float horizonLevelStrength) {
  // Applies only to axes that are in Angle mode
  // We now use Acro Rates, transformed into the range +/- 1, to provide
  // setpoints
  ...
  angleFeedforward = angleLimit * getFeedforward(axis) *
                     pidRuntime.angleFeedforwardGain * maxSetpointRateInv;
  //  angle feedforward must be heavily filtered, at the PID loop rate, with
  //  limited user control over time constant
  // it MUST be very delayed to avoid early overshoot and being too aggressive
  angleFeedforward =
      pt3FilterApply(&pidRuntime.angleFeedforwardPt3[axis], angleFeedforward);
#endif

  float angleTarget = angleLimit * currentPidSetpoint * maxSetpointRateInv;
  // use acro rates for the angle target in both horizon and angle modes,
  // converted to -1 to +1 range using maxRate
  ...
  angleTarget +=
      gpsRescueAngle[axis] / 100.0f; // Angle is in centidegrees, stepped on
                                     // roll at 10Hz but not on pitch
#endif
#if defined(USE_POSITION_HOLD) && !defined(USE_WING)
  if (FLIGHT_MODE(POS_HOLD_MODE)) {
    angleFeedforward = 0.0f; // otherwise the lag of the PT3 carries recent
                             // stick inputs into the hold
    if (isAutopilotInControl()) {
      // sticks are not deflected
      angleTarget = autopilotAngle[axis]; // autopilotAngle in degrees
      angleLimit =
          85.0f; // allow autopilot to use whatever angle it needs to stop
    }
    // limit pilot requested angle to half the autopilot angle to avoid excess
    // speed and chaotic stops
  ...
  const float currentAngle = (attitude.raw[axis] - angleTrim->raw[axis]) /
                             10.0f; // stepped at 500hz with some 4ms flat spots
  const float errorAngle = angleTarget - currentAngle;
  float angleRate = errorAngle * pidRuntime.angleGain + angleFeedforward;

  // minimise cross-axis wobble due to faster yaw responses than roll or pitch,
  // and make co-ordinated yaw turns by compensating for the effect of yaw on
  // roll while pitched, and on pitch while rolled earthRef code here takes
  // about 76 cycles, if conditional on angleEarthRef it takes about 100.
  // sin_approx costs most of those cycles.
  ...
  const float earthRefGain = FLIGHT_MODE(GPS_RESCUE_MODE | ALT_HOLD_MODE)
                                 ? 1.0f
                                 : pidRuntime.angleEarthRef;
  angleRate += pidRuntime.angleYawSetpoint * sinAngle * earthRefGain;
  pidRuntime.angleTarget[axis] =
      angleTarget; // set target for alternate axis to current axis, for use in
                   // preceding calculation

  // smooth final angle rate output to clean up attitude signal steps (500hz),
  // GPS steps (10 or 100hz), RC steps etc this filter runs at
  // ATTITUDE_CUTOFF_HZ, currently 50hz, so GPS roll may be a bit steppy
  ...
  if (FLIGHT_MODE(ANGLE_MODE | GPS_RESCUE_MODE | POS_HOLD_MODE)) {
    ...
    currentPidSetpoint = currentPidSetpoint * (1.0f - horizonLevelStrength) +
  ...
  if (axis == FD_ROLL) {
    ...
    DEBUG_SET(DEBUG_ANGLE_MODE, 1,
              lrintf(errorAngle * pidRuntime.angleGain *
                     10.0f)); // un-smoothed error correction in degrees
    DEBUG_SET(
        DEBUG_ANGLE_MODE, 2,
        lrintf(angleFeedforward * 10.0f)); // feedforward amount in degrees
    DEBUG_SET(DEBUG_ANGLE_MODE, 3,
              lrintf(currentAngle * 10.0f)); // angle returned

    DEBUG_SET(DEBUG_ANGLE_TARGET, 0, lrintf(angleTarget * 10.0f));
    DEBUG_SET(DEBUG_ANGLE_TARGET, 1,
              lrintf(sinAngle * 10.0f)); // modification factor from earthRef
    // debug ANGLE_TARGET 2 is yaw attenuation
    DEBUG_SET(DEBUG_ANGLE_TARGET, 3,
              lrintf(currentAngle * 10.0f)); // angle returned
  }

  DEBUG_SET(DEBUG_CURRENT_ANGLE, axis,
            lrintf(currentAngle * 10.0f)); // current angle
  return currentPidSetpoint;
}

static FAST_CODE_NOINLINE void
handleCrashRecovery(const pidCrashRecovery_e crash_recovery,
                    const rollAndPitchTrims_t *angleTrim, const int axis,
                    const timeUs_t currentTimeUs, const float gyroRate,
                    float *currentPidSetpoint, float *errorRate) {
  if (pidRuntime.inCrashRecoveryMode &&
      cmpTimeUs(currentTimeUs, pidRuntime.crashDetectedAtUs) >
          pidRuntime.crashTimeDelayUs) {
    if (crash_recovery == PID_CRASH_RECOVERY_BEEP) {
      BEEP_ON;
    }
    if (axis == FD_YAW) {
      *errorRate = constrainf(*errorRate, -pidRuntime.crashLimitYaw,
                              pidRuntime.crashLimitYaw);
    } else {
      // on roll and pitch axes calculate currentPidSetpoint and errorRate to
      // level the aircraft to recover from crash
    ...
    // and iterm windup during crash recovery can be extreme, especially on yaw
    // axis
    pidData[axis].I = 0.0f;
    if (cmpTimeUs(currentTimeUs, pidRuntime.crashDetectedAtUs) >
            pidRuntime.crashTimeLimitUs ||
      ...
      if (sensors(SENSOR_ACC)) {
        ...
        if (abs(attitude.raw[FD_ROLL] - angleTrim->raw[FD_ROLL]) <
                pidRuntime.crashRecoveryAngleDeciDegrees &&
            abs(attitude.raw[FD_PITCH] - angleTrim->raw[FD_PITCH]) <
                pidRuntime.crashRecoveryAngleDeciDegrees) {
...
static FAST_CODE_NOINLINE void
detectAndSetCrashRecovery(const pidCrashRecovery_e crash_recovery,
                          const int axis, const timeUs_t currentTimeUs,
                          const float delta, const float errorRate) {
  // if crash recovery is on and accelerometer enabled and there is no gyro
  // overflow, then check for a crash no point in trying to recover if the crash
  // is so severe that the gyro overflows
...
int acroTrainerSign(float x) { return x > 0 ? 1 : -1; }
...
//    Manage the setPoint to control the gyro rate as the actual angle
//    approaches the limit (try to prevent overshoot)
// 3. If no potential overflow is detected, then return the original setPoint

// Use the FAST_CODE_NOINLINE directive to avoid this code from being inlined
// into ITCM RAM. We accept the performance decrease when Acro Trainer mode is
// active under the assumption that user is unlikely to be expecting ultimate
// flight performance at very high loop rates when in this mode.
static FAST_CODE_NOINLINE float
applyAcroTrainer(int axis, const rollAndPitchTrims_t *angleTrim,
                 float setPoint) {
  float ret = setPoint;
  ...
  if (!FLIGHT_MODE(ANGLE_MODE | HORIZON_MODE | GPS_RESCUE_MODE | ALT_HOLD_MODE |
    ...
    if ((pidRuntime.acroTrainerAxisState[axis] != 0) &&
        (pidRuntime.acroTrainerAxisState[axis] !=
         setpointSign)) { // stick has reversed - stop limiting
      pidRuntime.acroTrainerAxisState[axis] = 0;
    }

    // Limit and correct the angle when it exceeds the limit
    if ((fabsf(currentAngle) > pidRuntime.acroTrainerAngleLimit) &&
        (pidRuntime.acroTrainerAxisState[axis] == 0)) {
    ...
    if (pidRuntime.acroTrainerAxisState[axis] != 0) {
      ret = constrainf(
          ((pidRuntime.acroTrainerAngleLimit * angleSign) - currentAngle) *
              pidRuntime.acroTrainerGain,
          -ACRO_TRAINER_SETPOINT_LIMIT, ACRO_TRAINER_SETPOINT_LIMIT);
    } else {

      // Not currently over the limit so project the angle based on current
      // angle and gyro angular rate using a sliding window based on gyro rate
      // (faster rotation means larger window. If the projected angle exceeds
      // the limit then apply limiting to minimize overshoot. Calculate the
      // lookahead window by scaling proportionally with gyro rate from 0-500dps
      float checkInterval = constrainf(fabsf(gyro.gyroADCf[axis]) /
                                           ACRO_TRAINER_LOOKAHEAD_RATE_LIMIT,
                                       0.0f, 1.0f) *
                            pidRuntime.acroTrainerLookaheadTime;
      ...
      if ((fabsf(projectedAngle) > pidRuntime.acroTrainerAngleLimit) &&
        ...
        ret = ((pidRuntime.acroTrainerAngleLimit * projectedAngleSign) -
               projectedAngle) *
              pidRuntime.acroTrainerGain;
        resetIterm = true;
      }
...
static float accelerationLimit(int axis, float currentPidSetpoint) {
  ...
  if (fabsf(currentVelocity) > pidRuntime.maxVelocity[axis]) {
    currentPidSetpoint =
        (currentVelocity > 0)
            ? previousSetpoint[axis] + pidRuntime.maxVelocity[axis]
            : previousSetpoint[axis] - pidRuntime.maxVelocity[axis];
  }
...
static void rotateVector(float v[XYZ_AXIS_COUNT],
                         const float rotation[XYZ_AXIS_COUNT]) {
...
#if defined(USE_ABSOLUTE_CONTROL)
STATIC_UNIT_TESTED void applyAbsoluteControl(const int axis,
                                             const float gyroRate,
                                             float *currentPidSetpoint,
                                             float *itermErrorRate) {
  if (pidRuntime.acGain > 0 || debugMode == DEBUG_AC_ERROR) {
    const float setpointLpf =
        pt1FilterApply(&pidRuntime.acLpf[axis], *currentPidSetpoint);
    ...
    if (wasThrottleRaised()) {
      axisError[axis] =
          constrainf(axisError[axis] + acErrorRate * pidRuntime.dT,
                     -pidRuntime.acErrorLimit, pidRuntime.acErrorLimit);
      const float acCorrection =
          constrainf(axisError[axis] * pidRuntime.acGain, -pidRuntime.acLimit,
                     pidRuntime.acLimit);
...
STATIC_UNIT_TESTED void applyItermRelax(const int axis, const float iterm,
                                        ...
                                        float *currentPidSetpoint) {
  const float setpointLpf =
      pt1FilterApply(&pidRuntime.windupLpf[axis], *currentPidSetpoint);
  const float setpointHpf = fabsf(*currentPidSetpoint - setpointLpf);

  if (pidRuntime.itermRelax) {
    if (axis < FD_YAW || pidRuntime.itermRelax == ITERM_RELAX_RPY ||
        pidRuntime.itermRelax == ITERM_RELAX_RPY_INC) {
      float itermRelaxThreshold = ITERM_RELAX_SETPOINT_THRESHOLD;
      if (FLIGHT_MODE(ANGLE_MODE)) {
        itermRelaxThreshold *= 0.2f;
      }
      const float itermRelaxFactor =
          MAX(0, 1 - setpointHpf / itermRelaxThreshold);
...
void pidUpdateAirmodeLpf(float currentOffset) {
  ...
  offsetHpf =
      offsetHpf - pt1FilterApply(&pidRuntime.airmodeThrottleLpf2, offsetHpf);

  // During high frequency oscillation 2 * currentOffset averages to the offset
  // required to avoid mirroring of the waveform
  ...
  if (currentOffset * pidRuntime.airmodeThrottleLpf1.state >= 0 &&
      fabsf(currentOffset) > pidRuntime.airmodeThrottleLpf1.state) {
    pidRuntime.airmodeThrottleLpf1.state = currentOffset;
  }
  pidRuntime.airmodeThrottleLpf1.state =
      constrainf(pidRuntime.airmodeThrottleLpf1.state,
                 -pidRuntime.airmodeThrottleOffsetLimit,
                 pidRuntime.airmodeThrottleOffsetLimit);
}
...
static FAST_CODE_NOINLINE void disarmOnImpact(void) {
  ...
  if (wasThrottleRaised()
    ...
    // increase sensitivity by 50% when low and in altitude hold or failsafe
    // landing for more reliable disarm with gentle controlled landings
    float lowAltitudeSensitivity = 1.0f;
#ifdef USE_ALTITUDE_HOLD
    lowAltitudeSensitivity =
        (FLIGHT_MODE(ALT_HOLD_MODE) && isBelowLandingAltitude()) ? 1.5f : 1.0f;
#endif
    // and disarm if jerk exceeds threshold...
    if ((acc.jerkMagnitude * lowAltitudeSensitivity) >
        pidRuntime.landingDisarmThreshold) {
      // then disarm
      setArmingDisabled(
          ARMING_DISABLED_ARM_SWITCH); // NB: need a better message
      disarm(DISARM_REASON_LANDING);
      // note: threshold should be high enough to avoid unwanted disarms in the
      // air on throttle chops, eg around 10
    }
  }
...
#define LAUNCH_CONTROL_ANGLE_WINDOW                                            \
  10.0f // The remaining angle degrees where rate dampening starts
...
// should be a non-issue.
static FAST_CODE_NOINLINE float
applyLaunchControl(int axis, const rollAndPitchTrims_t *angleTrim) {
  float ret = 0.0f;

  // Scale the rates based on stick deflection only. Fixed rates with a max of
  // 100deg/sec reached at 50% stick deflection. This keeps the launch control
  // positioning consistent regardless of the user's rates.
  if ((axis == FD_PITCH) ||
      (pidRuntime.launchControlMode != LAUNCH_CONTROL_MODE_PITCHONLY)) {
    const float stickDeflection =
        constrainf(getRcDeflection(axis), -0.5f, 0.5f);
    ret = LAUNCH_CONTROL_MAX_RATE * stickDeflection * 2;
  }

#if defined(USE_ACC)
  // If ACC is enabled and a limit angle is set, then try to limit forward tilt
  // to that angle and slow down the rate as the limit is approached to reduce
  // overshoot
  if ((axis == FD_PITCH) && (pidRuntime.launchControlAngleLimit > 0) &&
      (ret > 0)) {
    const float currentAngle =
        (attitude.raw[axis] - angleTrim->raw[axis]) / 10.0f;
    if (currentAngle >= pidRuntime.launchControlAngleLimit) {
      ...
      const float angleDelta =
          pidRuntime.launchControlAngleLimit - currentAngle;
      if (angleDelta <= LAUNCH_CONTROL_ANGLE_WINDOW) {
        ret = scaleRangef(angleDelta, 0, LAUNCH_CONTROL_ANGLE_WINDOW,
                          LAUNCH_CONTROL_MIN_RATE, ret);
      }
    }
  }
#else
...
static float getTpaFactor(const pidProfile_t *pidProfile, int axis,
                          term_e term) {
  float tpaFactor = pidRuntime.tpaFactor;
...
NOINLINE static void calculateSpaValues(const pidProfile_t *pidProfile) {
  ...
  for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
    ...
    pidRuntime.spa[axis] =
        1.0f - smoothStepUpTransition(fabsf(currentRate),
                                      pidProfile->spa_center[axis],
                                      pidProfile->spa_width[axis]);
...
// Betaflight pid controller, which will be maintained in the future with
// additional features specialised for current (mini) multirotor usage. Based on
// 2DOF reference design (matlab)
void FAST_CODE pidController(const pidProfile_t *pidProfile,
  ...
  const bool isExternalAngleModeRequest =
  ...
  if (FLIGHT_MODE(ANGLE_MODE | HORIZON_MODE | GPS_RESCUE_MODE)) {
    ...
    if ((levelModeStartTimeUs == 0) ||
        (isExternalAngleModeRequest && !prevExternalAngleRequest)) {
  ...
  DEBUG_SET(DEBUG_ANTI_GRAVITY, 2,
            lrintf((1 + (pidRuntime.itermAccelerator /
                         pidRuntime.pidCoefficient[FD_PITCH].Ki)) *
                   1000));
  ...
  for (int axis = FD_ROLL; axis <= FD_YAW; ++axis) {
    ...
    if (debugMode == DEBUG_D_LPF && axis != FD_YAW) {
      const float delta =
          ...
          pidRuntime.pidFrequency / D_LPF_RAW_SCALE;
      previousRawGyroRateDterm[axis] = gyroRateDterm[axis];
      DEBUG_SET(DEBUG_D_LPF, axis,
                lrintf(delta)); // debug d_lpf 2 and 3 used for pre-TPA D
    }
  ...
  // ToDo: check if this can be reconstructed offline for rotating filter and if
  // so, remove the debug fit (0...2*pi) into int16_t (-32768 to 32767)
  ...
  for (flight_dynamics_index_t axis = FD_ROLL; axis <= FD_YAW; ++axis) {
    ...
    // When Race Mode is active PITCH control is also GYRO based in level or
    // horizon mode
#if defined(USE_ACC)
    ...
    if (axis < FD_YAW) {
      ...
      if (levelMode == LEVEL_MODE_RP) {
        // if earth referencing is requested, attenuate yaw axis setpoint when
        // pitched or rolled and send yawSetpoint to Angle code to modulate
        // pitch and roll code cost is 107 cycles when earthRef enabled, 20
        // otherwise, nearly all in cos_approx
        const float earthRefGain =
            FLIGHT_MODE(GPS_RESCUE_MODE) ? 1.0f : pidRuntime.angleEarthRef;
        if (earthRefGain) {
          pidRuntime.angleYawSetpoint = currentPidSetpoint;
          float maxAngleTargetAbs =
              earthRefGain * fmaxf(fabsf(pidRuntime.angleTarget[FD_ROLL]),
                                   fabsf(pidRuntime.angleTarget[FD_PITCH]));
          maxAngleTargetAbs *=
              (FLIGHT_MODE(HORIZON_MODE)) ? horizonLevelStrength : 1.0f;
          // reduce compensation whenever Horizon uses less levelling
          currentPidSetpoint *=
              cos_approx(DEGREES_TO_RADIANS(maxAngleTargetAbs));
          DEBUG_SET(DEBUG_ANGLE_TARGET, 2,
                    currentPidSetpoint); // yaw setpoint after attenuation
        }
      }
    }
#endif
    ...
    if ((axis != FD_YAW) && pidRuntime.acroTrainerActive &&
        !pidRuntime.inCrashRecoveryMode && !launchControlActive) {
      currentPidSetpoint =
          applyAcroTrainer(axis, angleTrim, currentPidSetpoint);
    }
#endif // USE_ACRO_TRAINER
    ...
    // It's not necessary to zero the set points for R/P because the PIDs will
    // be zeroed below
#ifdef USE_YAW_SPIN_RECOVERY
    ...
    const float gyroRate =
        gyro.gyroADCf[axis]; // Process variable from gyro output in deg/sec
#ifdef USE_CHIRP
    currentPidSetpoint += currentChirp;
#endif                                               // USE_CHIRP
    float errorRate = currentPidSetpoint - gyroRate; // r - y
#if defined(USE_ACC)
    handleCrashRecovery(pidProfile->crash_recovery, angleTrim, axis,
                        currentTimeUs, gyroRate, &currentPidSetpoint,
                        &errorRate);
#endif
    ...
    if (!launchControlActive && !pidRuntime.inCrashRecoveryMode) {
      applyItermRelax(axis, previousIterm, gyroRate, &itermErrorRate,
                      &currentPidSetpoint);
      errorRate = currentPidSetpoint - gyroRate;
    }
#endif
    ...
    pidData[axis].P = pidRuntime.pidCoefficient[axis].Kp * errorRate *
                      getTpaFactor(pidProfile, axis, TERM_P);
    if (axis == FD_YAW) {
...
#endif
    {
      ...
      if (axis == FD_YAW) {
        itermLimit =
            pidRuntime.itermLimitYaw; // windup fraction of pidSumLimitYaw
    ...
    float iTermChange =
        (Ki + pidRuntime.itermAccelerator) * pidRuntime.dT * itermErrorRate;
#ifdef USE_WING
    if (pidProfile->spa_mode[axis] != SPA_MODE_OFF) {
      // slowing down I-term change, or even making it zero if setpoint is high
      // enough
    ...
    pidData[axis].I =
        constrainf(previousIterm + iTermChange, -itermLimit, itermLimit);
    ...
    if (FLIGHT_MODE(ANGLE_MODE) && pidRuntime.axisInAngleMode[axis]) {
      ...
      // it will already have stick based feedforward applied in the input to
      // their angle setpoint a simple setpoint Delta can be used to for PID
      // feedforward element for motor lag on these axes however RC steps come
      // in, via angle setpoint and setpoint RC smoothing must have a cutoff
      // half normal to remove those steps completely the RC stepping does not
      // come in via the feedforward, which is very well smoothed already if
      // uncommented, and the forcing to zero is removed, the two following
      // lines will restore PID feedforward to angle mode axes but for now let's
      // see how we go without it (which was the case before 4.5 anyway)
      //            pidSetpointDelta = currentPidSetpoint -
      //            pidRuntime.previousPidSetpoint[axis]; pidSetpointDelta *=
      //            pidRuntime.pidFrequency * pidRuntime.angleFeedforwardGain;
      pidSetpointDelta = 0.0f;
    } else {
      // the axis is operating as a normal acro axis, so use normal feedforard
      // from rc.c
      pidSetpointDelta = getFeedforward(axis);
    }
#endif
    pidRuntime.previousPidSetpoint[axis] =
        currentPidSetpoint; // this is the value sent to blackbox, and used for
                            // D-max setpoint
    ...
    if ((pidRuntime.pidCoefficient[axis].Kd > 0) && !launchControlActive) {
      ...
      const float delta = -(gyroRateDterm[axis] - previousGyroRateDterm[axis]) *
                          pidRuntime.pidFrequency;
      float preTpaD = pidRuntime.pidCoefficient[axis].Kd * delta;

#if defined(USE_ACC)
      if (cmpTimeUs(currentTimeUs, levelModeStartTimeUs) >
          CRASH_RECOVERY_DETECTION_DELAY_US) {
        detectAndSetCrashRecovery(pidProfile->crash_recovery, axis,
      ...
      if (pidRuntime.dMaxPercent[axis] > 1.0f) {
        float dMaxGyroFactor =
            pt2FilterApply(&pidRuntime.dMaxRange[axis], delta);
        dMaxGyroFactor = fabsf(dMaxGyroFactor) * pidRuntime.dMaxGyroGain;
        const float dMaxSetpointFactor =
            fabsf(pidSetpointDelta) * pidRuntime.dMaxSetpointGain;
        const float dMaxBoost = fmaxf(dMaxGyroFactor, dMaxSetpointFactor);
        // dMaxBoost starts at zero, and by 1.0 we get Dmax, but it can
        // exceed 1.
        dMaxMultiplier += (pidRuntime.dMaxPercent[axis] - 1.0f) * dMaxBoost;
        dMaxMultiplier =
            pt2FilterApply(&pidRuntime.dMaxLowpass[axis], dMaxMultiplier);
        ...
        if (debugMode == DEBUG_D_MAX && axis == gyro.gyroDebugAxis) {
          ...
          DEBUG_SET(DEBUG_D_MAX, 2,
                    lrintf(pidRuntime.pidCoefficient[axis].Kd * dMaxMultiplier *
                           10 / DTERM_SCALE)); // effective Kd after Dmax boost
      ...
      if (axis != FD_YAW) {
        DEBUG_SET(DEBUG_D_LPF, axis - FD_ROLL + 2,
                  lrintf(preTpaD * D_LPF_PRE_TPA_SCALE));
      }
    } else {
      pidData[axis].D = 0;
      if (axis != FD_YAW) {
    ...
    pidSetpointDelta +=
        setpointCorrection - pidRuntime.oldSetpointCorrection[axis];
    pidRuntime.oldSetpointCorrection[axis] = setpointCorrection;
#endif
    // no feedforward in launch control
    const float feedforwardGain =
        launchControlActive ? 0.0f : pidRuntime.pidCoefficient[axis].Kf;
    ...
    if (launchControlActive) {
      ...
      const int launchControlYawItermLimit =
          (pidRuntime.launchControlMode == LAUNCH_CONTROL_MODE_FULL)
              ? LAUNCH_CONTROL_YAW_ITERM_LIMIT
              : 0;
      pidData[FD_YAW].I =
          constrainf(pidData[FD_YAW].I, -launchControlYawItermLimit,
      ...
      if (pidRuntime.launchControlMode == LAUNCH_CONTROL_MODE_PITCHONLY) {
        ...
        pidData[FD_ROLL].I = 0;
        pidData[FD_YAW].P = 0;
        // don't let I go negative (pitch backwards) as front motors are limited
        // in the mixer
    ...
    pidData[axis].S =
        getSterm(axis, pidProfile, currentPidSetpointBeforeWingAdjust);
    applySpa(axis, pidProfile);

    // calculating the PID sum
    const float pidSum = pidData[axis].P + pidData[axis].I + pidData[axis].D +
                         pidData[axis].F + pidData[axis].S;
#ifdef USE_INTEGRATED_YAW_CONTROL
    if (axis == FD_YAW && pidRuntime.useIntegratedYaw) {
      pidData[axis].Sum += pidSum * pidRuntime.dT * 100.0f;
      pidData[axis].Sum -= pidData[axis].Sum * pidRuntime.integratedYawRelax /
                           100000.0f * pidRuntime.dT / 0.000125f;
    } else
#endif
    {
...
#ifdef USE_WING
  // When PASSTHRU_MODE is active - reset all PIDs to zero so the aircraft won't
  // snap out of control because of accumulated PIDs once PASSTHRU_MODE gets
  // disabled.
  bool isFixedWingAndPassthru = isFixedWing() && FLIGHT_MODE(PASSTHRU_MODE);
#endif // USE_WING
  // Disable PID control if at zero throttle or if gyro overflow detected
  // This may look very innefficient, but it is done on purpose to always show
  // real CPU usage as in flight
...
bool crashRecoveryModeActive(void) { return pidRuntime.inCrashRecoveryMode; }
...
bool pidAntiGravityEnabled(void) { return pidRuntime.antiGravityEnabled; }
...
void dynLpfDTermUpdate(float throttle) {
  if (pidRuntime.dynLpfFilter != DYN_LPF_NONE) {
    ...
    if (pidRuntime.dynLpfCurveExpo > 0) {
      cutoffFreq =
          dynLpfCutoffFreq(throttle, pidRuntime.dynLpfMin, pidRuntime.dynLpfMax,
    ...
    case DYN_LPF_BIQUAD:
      for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
        biquadFilterUpdateLPF(&pidRuntime.dtermLowpass[axis].biquadFilter,
                              cutoffFreq, targetPidLooptime);
      }
      break;
    case DYN_LPF_PT2:
      for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
        pt2FilterUpdateCutoff(&pidRuntime.dtermLowpass[axis].pt2Filter,
                              pt2FilterGain(cutoffFreq, pidRuntime.dT));
      }
      break;
    case DYN_LPF_PT3:
      for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
        pt3FilterUpdateCutoff(&pidRuntime.dtermLowpass[axis].pt3Filter,
                              pt3FilterGain(cutoffFreq, pidRuntime.dT));
      }
      break;
    }
  }
...
float dynLpfCutoffFreq(float throttle, uint16_t dynLpfMin, uint16_t dynLpfMax,
                       uint8_t expo) {
...
float pidGetDT(void) { return pidRuntime.dT; }

float pidGetPidFrequency(void) { return pidRuntime.pidFrequency; }

#ifdef USE_CHIRP
bool pidChirpIsFinished(void) { return pidRuntime.chirp.isFinished; }
#endif
